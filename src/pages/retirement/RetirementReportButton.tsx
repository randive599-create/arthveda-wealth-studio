/**
 * Download Retirement Plan Report button.
 *
 * Generates the retirement-specific PDF entirely client-side, reusing the
 * existing PDF engine. The heavy report module (jsPDF) is dynamically imported
 * on click so it stays out of the initial bundle. Announces the outcome via an
 * aria-live region.
 */
...
export interface RetirementReportButtonProps {
  inputs: RetirementInputs;
  result: RetirementResult;
}

export function RetirementReportButton({ inputs, result }: RetirementReportButtonProps) {
  ...
  const handleClick = useCallback(async () => {
    ...
    setMessage('');
    try {
      const { generateRetirementReport } = await import('./retirementReport');
      const fileName = await generateRetirementReport(inputs, result);
      setStatus('success');
  ...
  return (
    <div className="flex flex-col items-stretch gap-2 sm:items-end">
      <button
        ...
        data-testid={TESTIDS.retirementReportButton}
        className="
      ...
      >
        ...
        {generating ? 'Generating…' : 'Download Retirement Report'}
      </button>
      ...
      <div aria-live="polite" className="min-h-[1.25rem] text-right">
        {status === 'success' ? (
          <p
            ...
            data-testid={TESTIDS.retirementReportSuccess}
          >
        ...
        {status === 'error' ? (
          <p
            ...
            data-testid={TESTIDS.retirementReportError}
          >
