/**
 * SIP Projection Report — a dedicated, SIP-specific PDF.
 *
 * Reuses the existing PDF engine (jsPDF, the embedded Unicode report fonts and
 * the shared A4 page geometry) but renders SIP-specific content with the
 * required cover title "SIP Projection Report" — and includes the SIP amount,
 * duration, return assumption, maturity value and wealth created. No
 * retirement-specific wording. jsPDF is dynamically imported so it stays out of
 * the initial bundle.
 */
...
export function buildSipReportFileName(date: Date): string {
  return `ArthVeda-SIP-Projection-${isoDate(date)}.pdf`;
}
...
function renderHeader(doc: Doc, inputs: SipInputs, result: SipResult, generated: string): void {
  ...
  // Required SIP-specific report title.
  doc.setFont(REPORT_FONT_SERIF, 'bold');
  doc.setFontSize(26);
  doc.setTextColor(INK);
  doc.text('SIP Projection Report', MARGIN.left, MARGIN.top + 66);
  doc.setFont(REPORT_FONT_MONO, 'normal');
  doc.setFontSize(10);
  doc.setTextColor(INK_SECONDARY);
  doc.text('Systematic Investment Plan', MARGIN.left, MARGIN.top + 84);

  doc.setFont(REPORT_FONT_MONO, 'bold');
  doc.setFontSize(11);
  doc.setTextColor(ACCENT);
  doc.text(
    `${formatCurrency(inputs.monthlySip, result.currency)}/mo · ${inputs.durationYears} years · ${formatPercent(inputs.expectedReturnPct)}`,
    MARGIN.left,
    MARGIN.top + 104,
  );
  doc.setFont(REPORT_FONT_MONO, 'normal');
...
function renderProjectionTable(w: Writer, result: SipResult): void {
  const doc = w.doc;
  const rightEdge = A4.width - MARGIN.right;
  const cols = [
    { label: 'Year', x: MARGIN.left + 4, align: 'left' as const },
    { label: 'Invested', x: MARGIN.left + contentWidth() * 0.45, align: 'right' as const },
    { label: 'Corpus', x: MARGIN.left + contentWidth() * 0.72, align: 'right' as const },
    { label: 'Wealth Created', x: rightEdge - 4, align: 'right' as const },
  ];
  ...
  for (const point of result.series) {
    ...
    doc.setFont(REPORT_FONT_MONO, 'normal');
    doc.setTextColor(INK_SECONDARY);
    doc.text(money(point.invested), cols[1].x, w.y + 11, { align: cols[1].align });
    doc.setTextColor(INK);
    doc.text(money(point.corpus), cols[2].x, w.y + 11, { align: cols[2].align });
    doc.setTextColor(ACCENT);
    doc.text(money(point.gain), cols[3].x, w.y + 11, { align: cols[3].align });
    w.y += 16;
  }
}
...
function stampFooters(doc: Doc): void {
  ...
  for (let page = 1; page <= total; page += 1) {
    ...
    doc.setFontSize(6.5);
    doc.setTextColor(INK_SECONDARY);
    doc.text('Illustrative SIP projection only. Not investment advice.', leftX, y);
...
export interface GenerateSipReportOptions {
  now?: Date;
}

/**
 * Generate and save the SIP Projection Report. Returns the resolved file name.
 * Throws if PDF rendering fails (the caller surfaces it in the UI).
 */
export async function generateSipReport(
  inputs: SipInputs,
  result: SipResult,
  options: GenerateSipReportOptions = {},
): Promise<string> {
  ...
  renderHeader(typed, inputs, result, longDate(now));

  const money = (v: number) => formatCurrency(v, result.currency);
  const w = new Writer(typed);
  w.y = MARGIN.top + 140;

  // Plan summary — the required SIP fields + headline outputs.
  w.heading('SIP Projection Summary', 'Section 1');
  w.pairs([
    { label: 'SIP Amount', value: `${money(inputs.monthlySip)}/mo` },
    { label: 'Duration', value: `${inputs.durationYears} years` },
    { label: 'Return Assumption', value: formatPercent(inputs.expectedReturnPct) },
    { label: 'Total Invested', value: money(result.totalInvested) },
  ...
  w.heading('Assumptions', 'Section 2');
  w.pairs([
    { label: 'Annual SIP Step-Up', value: formatPercent(inputs.annualStepUpPct) },
    { label: 'Inflation', value: formatPercent(inputs.inflationPct) },
    { label: 'Existing Corpus', value: money(inputs.existingCorpus) },
    { label: 'Lumpsum Investment', value: money(inputs.lumpsumInvestment) },
  ...
  const fileName = buildSipReportFileName(now);
  typed.save(fileName);
  return fileName;
}
