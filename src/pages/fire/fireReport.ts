/**
 * FIRE Plan Report — a dedicated, FIRE-specific PDF.
 *
 * Reuses the existing PDF engine (jsPDF, the embedded Unicode report fonts, and
 * the shared A4 page geometry) but renders FIRE-specific content with a
 * FIRE-specific cover title: the plan summary (the six FIRE outputs), the plan
 * assumptions (the nine FIRE inputs), and a year-by-year projection of corpus
 * vs. FIRE number. jsPDF is dynamically imported so it stays out of the initial
 * bundle (only paid when a user downloads a report).
 */
...
/** Format a YYYY-MM-DD date and a human-readable long date. */
...
export function buildFireReportFileName(date: Date): string {
  return `ArthVeda-FIRE-Plan-${isoDate(date)}.pdf`;
}
...
class Writer {
  ...
  pairs(rows: Pair[]): void {
    ...
    for (const row of rows) {
      ...
      this.doc.setFontSize(10);
      this.doc.setTextColor(row.emphasis === 'accent' ? ACCENT : row.emphasis === 'risk' ? RISK : INK);
...
function renderHeader(doc: Doc, result: FireResult, fireAge: number, generated: string): void {
  ...
  doc.setFontSize(26);
  doc.setTextColor(INK);
  doc.text('FIRE Plan Report', MARGIN.left, MARGIN.top + 66);
  doc.setFont(REPORT_FONT_MONO, 'normal');
  doc.setFontSize(10);
  doc.setTextColor(INK_SECONDARY);
  doc.text('Financial Independence, Retire Early', MARGIN.left, MARGIN.top + 84);

  // Verdict line.
  doc.setFont(REPORT_FONT_MONO, 'bold');
  doc.setFontSize(11);
  doc.setTextColor(result.onTrack ? ACCENT : RISK);
  doc.text(
    result.onTrack ? `On track for FIRE at age ${fireAge}` : `Shortfall at the target FIRE age ${fireAge}`,
    MARGIN.left,
    MARGIN.top + 104,
  );
  doc.setFont(REPORT_FONT_MONO, 'normal');
...
/** Render the year-by-year corpus vs. FIRE-number projection table. */
function renderProjectionTable(w: Writer, result: FireResult): void {
  const doc = w.doc;
  const rightEdge = A4.width - MARGIN.right;
  const cols = [
    { label: 'Age', x: MARGIN.left + 4, align: 'left' as const },
    { label: 'Projected Corpus', x: MARGIN.left + contentWidth() * 0.52, align: 'right' as const },
    { label: 'FIRE Number', x: MARGIN.left + contentWidth() * 0.78, align: 'right' as const },
  ...
  for (const point of result.series) {
    ...
    const ahead = point.corpus >= point.fireNumber;
    doc.setFont(REPORT_FONT_MONO, 'bold');
    doc.setFontSize(8);
    doc.setTextColor(INK);
    doc.text(String(point.age), cols[0].x, w.y + 11, { align: cols[0].align });
    doc.setFont(REPORT_FONT_MONO, 'normal');
    doc.setTextColor(INK);
    doc.text(money(point.corpus), cols[1].x, w.y + 11, { align: cols[1].align });
    doc.setTextColor(INK_SECONDARY);
    doc.text(money(point.fireNumber), cols[2].x, w.y + 11, { align: cols[2].align });
    doc.setFont(REPORT_FONT_MONO, 'bold');
    doc.setTextColor(ahead ? ACCENT : RISK);
    doc.text(ahead ? 'FIRE' : 'Gap', cols[3].x, w.y + 11, { align: cols[3].align });
    w.y += 16;
  }
}

/** Stamp the repeating footer on every page. */
function stampFooters(doc: Doc): void {
  ...
  for (let page = 1; page <= total; page += 1) {
    ...
    doc.setFontSize(6.5);
    doc.setTextColor(INK_SECONDARY);
    doc.text('Illustrative FIRE projection only. Not investment advice.', leftX, y);
...
export interface GenerateFireReportOptions {
  now?: Date;
}

/**
 * Generate and save the FIRE Plan Report. Returns the resolved file name.
 * Throws if PDF rendering fails (the caller surfaces it in the UI).
 */
export async function generateFireReport(
  inputs: FireInputs,
  result: FireResult,
  options: GenerateFireReportOptions = {},
): Promise<string> {
  ...
  // Plan summary — the six FIRE outputs.
  w.heading('FIRE Plan Summary', 'Section 1');
  w.pairs([
    { label: 'FIRE Number Required (today)', value: money(result.fireNumberToday) },
    {
      label: `Inflation-Adjusted FIRE Number (age ${fireAge})`,
      value: money(result.inflationAdjustedFireNumber),
    },
    { label: 'Projected Corpus at FIRE Age', value: money(result.projectedCorpusAtFire) },
    { label: 'Years Remaining to FIRE', value: `${result.yearsToFire} years` },
    {
      label: 'FIRE Success Indicator',
      value: result.onTrack ? 'On Track' : 'Shortfall',
      emphasis: result.onTrack ? 'accent' : 'risk',
    },
    {
  ...
  // Plan assumptions — the nine FIRE inputs.
  w.heading('Plan Assumptions', 'Section 2');
  w.pairs([
    { label: 'Current Age', value: `${inputs.currentAge} years` },
    { label: 'Target FIRE Age', value: `${inputs.targetFireAge} years` },
    { label: 'Current Net Worth', value: money(inputs.currentNetWorth) },
    ...
    { label: 'Annual Expenses Today', value: money(inputs.annualExpensesToday) },
    { label: 'Safe Withdrawal Rate', value: formatPercent(inputs.safeWithdrawalRatePct) },
  ]);

  // Year-by-year corpus vs FIRE number.
  w.heading('Year-by-Year Projection', 'Section 3');
  renderProjectionTable(w, result);

  stampFooters(typed);

  const fileName = buildFireReportFileName(now);
  typed.save(fileName);
  return fileName;
}
