/**
 * Download FIRE Plan Report button.
 *
 * Generates the FIRE-specific PDF entirely client-side, reusing the existing
 * PDF engine. The heavy report module (jsPDF) is dynamically imported on click
 * so it stays out of the initial bundle. Announces progress/outcome via an
 * aria-live region.
 */
...
export interface FireReportButtonProps {
  inputs: FireInputs;
  result: FireResult;
}

export function FireReportButton({ inputs, result }: FireReportButtonProps) {
  ...
  const handleClick = useCallback(async () => {
    ...
    setMessage('');
    try {
      const { generateFireReport } = await import('./fireReport');
      const fileName = await generateFireReport(inputs, result);
      setStatus('success');
  ...
  return (
    <div className="flex flex-col items-stretch gap-2 sm:items-end">
      <button
        ...
        data-testid={TESTIDS.fireReportButton}
        className="
      ...
      >
        ...
        {generating ? 'Generating…' : 'Download FIRE Report'}
      </button>
      ...
      <div aria-live="polite" className="min-h-[1.25rem] text-right">
        {status === 'success' ? (
          <p
            ...
            data-testid={TESTIDS.fireReportSuccess}
          >
        ...
        {status === 'error' ? (
          <p
            ...
            data-testid={TESTIDS.fireReportError}
          >
